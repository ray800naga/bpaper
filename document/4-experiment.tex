...
\section{単語の感情推定の妥当性に関する評価実験}
	...
	\subsection{実験結果}
		...
		表\ref{table:top-k_cos-sim_all}について，
		...
		表\ref{table:top-1_hinshi}について，
		データの正規性を確認するためにShapiro-Wilk検定を行うと,
		以下の表のようになる．

		\begin{table}[H]
			\centering
			\caption{表\ref{table:top-1_hinshi}のデータに対するShapiro-Wilk検定の結果}
			\label{table:SW_kentei_top-1_hinshi}
			\begin{tabular}{ccc}
				\hline
